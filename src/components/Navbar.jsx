...
const Navbar = () => {
  ...
  return (
    <div className="fixed z-[999] w-full px-6 md:px-20 py-4 md:py-8 font-['Neue Montreal'] flex justify-between items-center">
      ...
      <button
        className="md:hidden inline-flex items-center justify-center w-10 h-10 rounded-full border border-zinc-500"
        aria-label="Menu"
    ...
    {isMobileMenuOpen && (
      <div className="md:hidden fixed top-[64px] right-0 left-0 bg-zinc-900 border-t border-zinc-800 px-6 py-4 z-[998]">
        <div className='flex flex-col gap-4'>
          {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) => (
              <a key={index} className="text-base capitalize font-light py-1 border-b border-zinc-800/50 last:border-b-0">{item}</a>
          ))}
        </div>
      </div>
    )}
